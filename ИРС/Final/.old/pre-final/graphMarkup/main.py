...
class Window:
    ...
    def show(self):
        ...
        resultImg = self.drawInfo(resultImg)
    ...
    def drawInfo(self, img):
        ...
        if self.pointMode == 0: colorMode = self.roadColor
        elif self.pointMode == 1: colorMode = self.crossroadColor
        elif self.pointMode == 2: colorMode = self.arucoColor
        resultImg[:self.offset, w//3-(self.offset//2):w//3+(self.offset//2)] = colorMode
        textPos = (w*2//3, self.offset//2+10)
        textMode = 'Point' if self.drawMode else 'Line'
        cv2.putText(resultImg, textMode, textPos, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        return resultImg
...
def addLine(startPoint, endPoint):
    ...
    elif endPoint.id not in mapDict['graph'][startPoint.id]:
